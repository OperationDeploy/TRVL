...
const getTripActivities = async (req, res) => {
  ...
  const tripActivities = await TripItinerary.findAll({
    where: { trip_id: req.trip },
  });

  res.status(200).send(tripActivities);
  // .catch((err) => console.warn('Error getting trip activities from db', err));
};

const deleteActivity = async (req) => {
  await TripItinerary.destroy({
    where: {
      user_id: req.user, trip_id: req.trip_id, event: req.event },
  })
    .then(console.info('success deleting activity from db'))
    .catch((err) => console.warn('error deleting activity from db', err));
};
